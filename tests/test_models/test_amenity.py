...
class TestAmenity(unittest.TestCase):
    ...
    def test_instance_creation(self):
        """Test if a Amenity instance is created"""
        class_instance = Amenity()
        self.assertIsInstance(class_instance, Amenity)

    def test_str_method(self):
        """Test if the __str__ method is implemented"""
        expected_str = "[Amenity] ({}) {}".format(self.class_instance.id,
                                                  self.class_instance.__dict__)
        self.assertEqual(str(self.class_instance), expected_str)

    def test_attributes_values(self):
        """
        Ensure that a Amenity instance is created correctly
    ...
    def test_kwargs(self):
        """Tests user kwargs"""
        new_user = Amenity(**self.class_instance.to_dict())
